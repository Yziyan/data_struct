...
public class HashMap<K, V> implements Map<K, V> {
    ...
    private static final float DEFAULT_LOAD_FACTOR = 0.75f; // 装填因子临界点
    ...
    public V put(K key, V value) {
        ...
        if (root == null) { // 此桶第一次放置元素
            root = createNode(key, value, null); // 构建红黑树根节点
            table[index] = root; //
            ficAfterPut(root); // 维护红黑树性质
            size++;
            return null;
        }
        ...
        do {
            ...
            if (h1 > h2) {
                ...
                // 这里其实用什么样的规则都可以，但是为了未来调试方便，还是能够以一定的规则
    ...
    protected static class Node<K, V> {
        ...
        public Node(K key, V value, Node<K, V> parent) {
            ...
            this.hash = (hash >>> 16) ^ hash; // 直接存储扰动计算后的哈希值
            this.value = value;
            this.parent = parent;
        }
    ...
    private void resize() {
        ...
        table = new Node[oldTable.length << 1]; // 扩容为原来的两倍
     ...
     * @param node：新添加的节点
     */
    private void ficAfterPut(Node<K, V> node) {
        ...
        if (isLeft) { // 被删除节点位于左子树【与右子树操作对称】
            ...
            if (isRed(sibling.left) || isRed(sibling.right)) { // 兄弟节点至少有一个红色的子节点，可以借用
                ...
                if (parentBlack) { // 如果以前就是黑色，父节点向下合并后，也会参数下溢
                    fixAfterRemove(parent); // 将父节点当做被删除的节点，递归调用此函数
                }
            }
    ...
    private V remove(Node<K, V> node) {
        ...
        if (child != null) { // 删除节点的度为 1
            ...
            fixAfterRemove(child); // 修复红黑树性质【这里需要传入子节点】
        } else { // 删除节点的度为 0
            fixAfterRemove(node); // 修复红黑树性质
        }

        // 被删除的节点肯定是正确的了[用与子类实现的方法]
